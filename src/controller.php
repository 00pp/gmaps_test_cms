...
{	
	...
	/*	if ($page == null)
		...
		$states = json_decode($states,1);

		$model = $this -> db -> locationPage($state_id, $town_id, $category_id);
		...
		foreach ($item as $key => $value) {
			...
			$item[$key]['state_full_name'] = $item[$key]['state'];
		}
...
/*	public function tagPage($id, $page)
	{	

		if ($page == null)
			$page = 0;

		$model = $this -> db -> tagPage($id, $page);
		...
		$this->twig->display('tag.html.twig', 
			[
				'category' => $item, 
				'next_page' => $model['next_page'],
				'curr_page' => $page,
